...
// add states
content = content.replace(/const \[activeTab, setActiveTab\] = useState\('dinh_ky'\);/, 
  `const [activeTab, setActiveTab] = useState('dinh_ky');
...
content = content.replace(
  /<button onClick=\{.*?handleSaveToDrive\(cdResult, 'ke_hoach'\).*?>[\s\S]*?<\/button>/,
...
content = content.replace(/<\/div>\s*<\/div>\s*{\/\* CONTENT: KHO BIỂU MẪU \*\//, function(match) {
  return modalStr + '\n' + match;
});
